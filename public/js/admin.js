...
// --- SYSTEM MAINTENANCE: CLEAN TEST DATA HANDLER ---
window.triggerCleanTestData = async function() {
  if (!confirm("⚠️ CAUTION: Are you sure you want to permanently delete all test appointments, prescriptions, uploaded patient reports, and test patient accounts?\n\nDoctor profiles and medicine catalog will NOT be deleted.")) {
    return;
  }
  
  const confirmText = prompt("Type 'DELETE' to confirm wiping test data:");
  if (confirmText !== 'DELETE') {
    alert("Action cancelled.");
    return;
  }

  const token = localStorage.getItem('chc_token') || localStorage.getItem('token');
  const banner = document.getElementById('clean-data-status-banner');
  if (banner) {
    banner.style.display = 'block';
    banner.className = 'status-banner info';
    banner.textContent = 'Wiping test data and storage files... Please wait.';
  }
  ...
  try {
    const res = await fetch('/api/admin/clean-test-data', {
      ...
      body: JSON.stringify({ deletePatientUsers: true })
    });
    const data = await res.json();
    if (res.ok) {
      if (banner) {
        banner.className = 'status-banner success';
        banner.textContent = '✅ Cleanup successful! All test records and uploaded documents have been deleted.';
      }
      alert('✅ Test data successfully wiped!\n\nDatabase and Supabase storage are clean and ready for production.');
      if (typeof loadAdminDashboard === 'function') loadAdminDashboard();
    } else {
      if (banner) {
        banner.className = 'status-banner danger';
        banner.textContent = 'Error: ' + (data.error || 'Failed to wipe test data.');
      }
    }
  } catch (err) {
    if (banner) {
